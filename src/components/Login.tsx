...
import type { LoginDto, Organization, Location as LocationType } from '@/lib/api';
...
interface LoginProps {
  onLoginSuccess: (needsLocationSelection?: boolean, locations?: LocationType[]) => void;
  onCreateOrg: () => void;
}
...
export function Login({ onLoginSuccess, onCreateOrg }: LoginProps) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    setIsLoading(true);
    try {
      const { locations } = await authService.login({
        email: data.email,
      ...
      if (locations.length > 1) {
        // Multiple locations - needs selection
        onLoginSuccess(true, locations);
      } else if (locations.length === 1) {
        // Single location - auto-selected during login
        onLoginSuccess(false);
      } else {
        // No locations - proceed without location context
        onLoginSuccess(false);
      }
    } catch (error: any) {
